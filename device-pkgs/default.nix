    ...
    writeShellApplication {
      ...
      text =
        ...
        ''
          ...
          # TODO(eberman): stop assuming /dev/serial/by-id is present. It'll be populated with udev, so most Linux systems will work fine
          echo -n "Attempting to connect to serial gadget via usb, this may take up to 4 minutes"
          ...
          if [ ! -e /dev/serial/by-id/${serialPortId} ] ; then
            echo "Serial gadget didn't come up, please monitor the real serial port or wait 5-15 minutes"
            exit 2
          fi
